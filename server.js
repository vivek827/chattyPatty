...
io.on("connection", async (socket) => {
  ...
  socket.on("start", (id) => {
    ...
    while (i < searching.length) {
      ...
      if (peer.id !== id) {
        ...
        io.of("/")
          .to(roomName)
          .emit("chatStart", "You are now chatting with a random parul student");
  ...
  socket.on("stop", () => {
    ...
    peer.emit("strangerDisconnected", "Student has disconnected");
  ...
  socket.on("disconnecting", async () => {
    ...
    if (roomName) {
      io.of("/").to(roomName).emit("goodBye", "Student has disconnected");
